    ...
    <div className={cn("min-h-screen", isLight ? "bg-stone-50 text-stone-950" : "bg-zinc-950 text-zinc-50")}>
      ...
      >
        <div className="flex h-full flex-col">
          ...
          <nav className="mt-8 space-y-1">
            <DashboardNavigationLinks active={active} isLight={isLight} />
          </nav>
      ...
      >
        ...
        <MobileSideNavigation
          ariaLabel="Broker workspace navigation"
          brand={
          ...
          navigation={<div className="space-y-1"><DashboardNavigationLinks active={active} isLight={isLight} /></div>}
          footer={
            <div className="space-y-3">
              <p className={cn("px-2 text-xs", isLight ? "text-stone-500" : "text-zinc-500")}>Signed in to broker workspace</p>
              <SignOutForm />
            </div>
          }
          contentClassName={cn(
            isLight ? "border-stone-200 bg-white text-stone-950" : "border-white/10 bg-zinc-950 text-white",
          )}
        />
      </header>
...
function DashboardNavigationLinks({ active, isLight }: { active?: string; isLight: boolean }) {
  return nav.map((item) => {
